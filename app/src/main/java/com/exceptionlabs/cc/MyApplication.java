...
public class MyApplication extends Application {
    ...
    public void onCreate() {
        ...
        if (!lastSyncedUID.equals("0")) {
            if (!lastSyncedUID.equals(currentUID)) {
                databaseRef.getReference().child("/users/UID/" + lastSyncedUID).keepSynced(false);
            }
        }
        // Keep current account in sync
        if (!currentUID.equals("0")) {
            databaseRef.getReference().child("/users/UID/" + currentUID).keepSynced(true);
